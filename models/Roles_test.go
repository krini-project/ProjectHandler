...
func testRoleToManyProjectUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadProjectUsers(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyRightsRightIDRights(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, rightDBTypes, false, rightColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, rightDBTypes, false, rightColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	_, err = tx.Exec("insert into `Roles_has_Rights` (`Roles_RoleID`, `Rights_RightID`) values (?, ?)", a.RoleID, b.RightID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `Roles_has_Rights` (`Roles_RoleID`, `Rights_RightID`) values (?, ?)", a.RoleID, c.RightID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.RightsRightIDRights().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.RightID == b.RightID {
			bFound = true
		}
		if v.RightID == c.RightID {
			cFound = true
		}
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadRightsRightIDRights(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RightsRightIDRights); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.RightsRightIDRights = nil
	if err = a.L.LoadRightsRightIDRights(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RightsRightIDRights); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRoleToManyAddOpProjectUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.RoleID != first.RoleID {
			t.Error("foreign key was wrong value", a.RoleID, first.RoleID)
		}
		if a.RoleID != second.RoleID {
			t.Error("foreign key was wrong value", a.RoleID, second.RoleID)
		}
...
func testRoleToManyAddOpRightsRightIDRights(t *testing.T) {
	var err error
	...
	foreigners := []*Right{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, rightDBTypes, false, strmangle.SetComplement(rightPrimaryKeyColumns, rightColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Right{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddRightsRightIDRights(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if first.R.RolesRoleIDRoles[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
		if second.R.RolesRoleIDRoles[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}

		if a.R.RightsRightIDRights[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.RightsRightIDRights[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.RightsRightIDRights().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testRoleToManySetOpRightsRightIDRights(t *testing.T) {
	var err error
	...
	err = a.SetRightsRightIDRights(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetRightsRightIDRights(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	//
	// if len(b.R.RolesRoleIDRoles) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	// if len(c.R.RolesRoleIDRoles) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	if d.R.RolesRoleIDRoles[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
	if e.R.RolesRoleIDRoles[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}

	if a.R.RightsRightIDRights[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.RightsRightIDRights[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testRoleToManyRemoveOpRightsRightIDRights(t *testing.T) {
	var err error
	...
	err = a.AddRightsRightIDRights(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveRightsRightIDRights(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.RightsRightIDRights) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.RightsRightIDRights[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.RightsRightIDRights[0] != &e {
...
var (
	roleDBTypes = map[string]string{`RoleID`: `int`, `RoleName`: `varchar`}
	_           = bytes.MinRead
)
