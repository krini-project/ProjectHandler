...
type Role struct {
	...
	RoleName string `boil:"RoleName" json:"RoleName" toml:"RoleName" yaml:"RoleName"`
...
var RoleWhere = struct {
	...
	RoleID:   whereHelperint{field: "`Roles`.`RoleID`"},
	RoleName: whereHelperstring{field: "`Roles`.`RoleName`"},
}

// RoleRels is where relationship names are stored.
var RoleRels = struct {
	ProjectUsers        string
	RightsRightIDRights string
}{
	ProjectUsers:        "ProjectUsers",
	RightsRightIDRights: "RightsRightIDRights",
}

// roleR is where relationships are stored.
type roleR struct {
	ProjectUsers        ProjectUserSlice
	RightsRightIDRights RightSlice
}
...
var (
	roleAllColumns            = []string{"RoleID", "RoleName"}
	roleColumnsWithoutDefault = []string{"RoleName"}
	roleColumnsWithDefault    = []string{"RoleID"}
	rolePrimaryKeyColumns     = []string{"RoleID"}
)
...
// ProjectUsers retrieves all the ProjectUser's ProjectUsers with an executor.
func (o *Role) ProjectUsers(mods ...qm.QueryMod) projectUserQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`ProjectUser`.`Role_id`=?", o.RoleID),
	)
...
// RightsRightIDRights retrieves all the Right's Rights with an executor via RightID column.
func (o *Role) RightsRightIDRights(mods ...qm.QueryMod) rightQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`Roles_has_Rights` on `Rights`.`RightID` = `Roles_has_Rights`.`Rights_RightID`"),
		qm.Where("`Roles_has_Rights`.`Roles_RoleID`=?", o.RoleID),
	)

	query := Rights(queryMods...)
	queries.SetFrom(query.Query, "`Rights`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`Rights`.*"})
	}
...
func (roleL) LoadProjectUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(qm.From(`ProjectUser`), qm.WhereIn(`ProjectUser.Role_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadRightsRightIDRights allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadRightsRightIDRights(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.Select("`Rights`.*, `a`.`Roles_RoleID`"),
		qm.From("`Rights`"),
		qm.InnerJoin("`Roles_has_Rights` as `a` on `Rights`.`RightID` = `a`.`Rights_RightID`"),
		qm.WhereIn("`a`.`Roles_RoleID` in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load Rights")
	}

	var resultSlice []*Right
	...
	for results.Next() {
		...
		err = results.Scan(&one.RightID, &one.RightName, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for Rights")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice Rights")
		}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on Rights")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for Rights")
	}
	...
	if singular {
		object.R.RightsRightIDRights = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &rightR{}
			}
			foreign.R.RolesRoleIDRoles = append(foreign.R.RolesRoleIDRoles, object)
		}
		return nil
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.RoleID == localJoinCol {
				local.R.RightsRightIDRights = append(local.R.RightsRightIDRights, foreign)
				if foreign.R == nil {
					foreign.R = &rightR{}
				}
				foreign.R.RolesRoleIDRoles = append(foreign.R.RolesRoleIDRoles, local)
				break
			}
		}
	}
...
func (o *Role) AddProjectUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ProjectUser) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.RoleID, rel.RoleID, rel.UserID}
...
// AddRightsRightIDRights adds the given related objects to the existing relationships
// of the Role, optionally inserting them as new records.
// Appends related to o.R.RightsRightIDRights.
// Sets related.R.RolesRoleIDRoles appropriately.
func (o *Role) AddRightsRightIDRights(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Right) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into `Roles_has_Rights` (`Roles_RoleID`, `Rights_RightID`) values (?, ?)"
		values := []interface{}{o.RoleID, rel.RightID}
	...
	if o.R == nil {
		o.R = &roleR{
			RightsRightIDRights: related,
		}
	} else {
		o.R.RightsRightIDRights = append(o.R.RightsRightIDRights, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &rightR{
				RolesRoleIDRoles: RoleSlice{o},
			}
		} else {
			rel.R.RolesRoleIDRoles = append(rel.R.RolesRoleIDRoles, o)
		}
	}
	return nil
}

// SetRightsRightIDRights removes all previously related items of the
// Role replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.RolesRoleIDRoles's RightsRightIDRights accordingly.
// Replaces o.R.RightsRightIDRights with related.
// Sets related.R.RolesRoleIDRoles's RightsRightIDRights accordingly.
func (o *Role) SetRightsRightIDRights(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Right) error {
	query := "delete from `Roles_has_Rights` where `Roles_RoleID` = ?"
	...
	removeRightsRightIDRightsFromRolesRoleIDRolesSlice(o, related)
	if o.R != nil {
		o.R.RightsRightIDRights = nil
	}
	return o.AddRightsRightIDRights(ctx, exec, insert, related...)
}

// RemoveRightsRightIDRights relationships from objects passed in.
// Removes related items from R.RightsRightIDRights (uses pointer comparison, removal does not keep order)
// Sets related.R.RolesRoleIDRoles.
func (o *Role) RemoveRightsRightIDRights(ctx context.Context, exec boil.ContextExecutor, related ...*Right) error {
	var err error
	query := fmt.Sprintf(
		"delete from `Roles_has_Rights` where `Roles_RoleID` = ? and `Rights_RightID` in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.RoleID}
	for _, rel := range related {
		values = append(values, rel.RightID)
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.RightsRightIDRights {
			if rel != ri {
				continue
			}

			ln := len(o.R.RightsRightIDRights)
			if ln > 1 && i < ln-1 {
				o.R.RightsRightIDRights[i] = o.R.RightsRightIDRights[ln-1]
			}
			o.R.RightsRightIDRights = o.R.RightsRightIDRights[:ln-1]
			break
		}
	}
...
func removeRightsRightIDRightsFromRolesRoleIDRolesSlice(o *Role, related []*Right) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.RolesRoleIDRoles {
			if o.RoleID != ri.RoleID {
				continue
			}

			ln := len(rel.R.RolesRoleIDRoles)
			if ln > 1 && i < ln-1 {
				rel.R.RolesRoleIDRoles[i] = rel.R.RolesRoleIDRoles[ln-1]
			}
			rel.R.RolesRoleIDRoles = rel.R.RolesRoleIDRoles[:ln-1]
			break
		}
	}
}
...
func FindRole(ctx context.Context, exec boil.ContextExecutor, roleID int, selectCols ...string) (*Role, error) {
	...
	query := fmt.Sprintf(
		"select %s from `Roles` where `RoleID`=?", sel,
	)
...
func (o *Role) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.RoleID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == roleMapping["RoleID"] {
		goto CacheNoHooks
	}
...
func RoleExists(ctx context.Context, exec boil.ContextExecutor, roleID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `Roles` where `RoleID`=? limit 1)"
